...
public:
	...
	Laborator(){
		...
		view_matrix_normal = glm::lookAt(glm::vec3(0,50,100), glm::vec3(0,10,0), glm::vec3(0,1,0));
        view_matrix_reflectiv = glm::lookAt(glm::vec3(0, 20, -60), glm::vec3(0, 10, 0), glm::vec3(0, 1, 0));

		//desenare wireframe
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

		framebuffer = lab::Framebuffer();
	}
	...
	void notifyDisplayFrame(){
		...
		framebuffer.reshape(screen_width, screen_height);
		framebuffer.bind();
        ...
        if (toggle_quad_texture){
			...
			//std::cout << framebuffer.getColorTexture();
        }
